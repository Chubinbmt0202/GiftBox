...
export function CartPreviewSidebar({
    ...
    return (
        <div className="bg-white rounded-[2rem] p-5 shadow-sm border border-gray-100/50 flex flex-col gap-5">
            ...
            {!selectedBox && totalItemsQuantity === 0 && !selectedCard && (
                <div className="text-center py-6 text-gray-400">
                    <p className="text-sm font-fredoka">Your box is empty</p>
                    <p className="text-xs mt-1">Start by choosing a box style</p>
                </div>
            )}

            <div className="flex flex-col gap-3 overflow-y-auto max-h-[400px] pr-2 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-gray-200 [&::-webkit-scrollbar-thumb]:rounded-full">
                {selectedBox && (
                    <div className="flex gap-3 items-center group relative p-2 -mx-2 rounded-xl hover:bg-gray-50 transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-500">Box Style</p>
                        </div>
                        <div className="text-right flex-shrink-0 flex flex-col items-end gap-1">
                            <p className="font-semibold text-gray-900 text-sm font-sans">{selectedBox.price.toLocaleString()}₫</p>
                            {currentStep > 1 && (
                                <button onClick={onChangeBox} className="text-[10px] font-bold text-boonie-pink hover:underline">Change</button>
                            )}
                        </div>
                    </div>
                )}
                ...
                {selectedItems.map(item => (
                    <div key={item.id} className="flex gap-3 items-center group relative p-2 -mx-2 rounded-xl hover:bg-gray-50 transition-colors">
                        <div className="w-12 h-12 bg-[#f4ede4] rounded-xl overflow-hidden flex-shrink-0 relative">
                            <img src={item.image} alt={item.name} className="w-full h-full object-cover mix-blend-multiply" />
                            <div className="absolute -top-1 -right-1 bg-gray-900 text-white w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-bold border border-white">
                                {item.quantity}
                            </div>
                        </div>
                        <div className="flex-1 min-w-0">
                        ...
                        <div className="text-right flex-shrink-0 flex items-center gap-2">
                            <p className="font-semibold text-gray-900 text-sm font-sans">{(item.price * item.quantity).toLocaleString()}₫</p>
                            <button
                                onClick={() => handleItemRemove(item.id)}
                                className="w-6 h-6 rounded-md bg-white border border-gray-100 flex items-center justify-center text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors opacity-0 group-hover:opacity-100"
                            >
                ...
                {selectedCard && (
                    <div className="flex gap-3 items-center group relative p-2 -mx-2 rounded-xl hover:bg-gray-50 transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-500">Greeting Card</p>
                        </div>
                        <div className="text-right flex-shrink-0">
                            <p className="font-semibold text-gray-900 text-sm font-sans">{selectedCard.price.toLocaleString()}₫</p>
                        </div>
                    </div>
                )}
            </div>
            ...
            {itemsNeeded > 0 && selectedBox && currentStep === 2 && (
                <div className="bg-blue-50 text-blue-600 p-2.5 rounded-xl text-xs font-medium text-center border border-blue-100">
                    Add {itemsNeeded} more item{itemsNeeded > 1 ? 's' : ''} to fill your box!
                </div>
            )}

            <div className="border-t border-gray-100 pt-3 mt-1">
                <div className="flex items-center justify-between mb-4">
                    <span className="text-gray-500 text-sm font-fredoka">Subtotal</span>
                    <span className="text-xl font-bold text-gray-900 font-sans">{totalPrice.toLocaleString()}₫</span>
                </div>

                {currentStep < 3 ? (
                    <button
                        onClick={onNext}
                        className={`w-full py-3 rounded-2xl font-bold text-sm transition-all shadow-sm ${(currentStep === 1 && !selectedBox) ||
                    ...
                    >
                        {currentStep === 1 ? 'Next: Add Items' : 'Next: Add a Note'}
                    </button>
                ) : (
                    <button
                        onClick={onComplete}
                        className={`w-full py-3 rounded-2xl font-bold text-sm transition-all shadow-sm ${!selectedCard
                                ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
